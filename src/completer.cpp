...
std::vector<Candidate> Completer::complete(const std::string &prefix) {
    ...
    while (left <= right) {
        ...
        if (key < prefix) {
            ...
            // else if (data[mid][0].startsWith(prefix)) { firstMatch = mid; right = mid - 1; }
            // else { right = mid - 1; }
            ...
            // The TS logic relies on the fact that if it starts with prefix, it is >= prefix.
            // But "foobar" > "foo".
            ...
            if (key < prefix && !startsWith) { 
                 // Strictly less and not a prefix match (which is impossible if strictly less, unless prefix is empty?)
                 // If prefix is "a", key "a" is not < "a".
                 // If prefix is "ab", key "a" < "ab". "a" does not start with "ab".
                 left = mid + 1;
            } else if (startsWith) {
                firstMatch = mid;
                right = mid - 1;
    ...
    */
    ...
    while (left <= right) {
        ...
        // Check startsWith
        bool startsWith = key.size() >= prefix.size() && key.compare(0, prefix.size(), prefix) == 0;
        
        if (key < prefix && !startsWith) {
             // Note: if key starts with prefix, it must be >= prefix (e.g. "apple" >= "app").
             // So if key < prefix, it cannot start with prefix.
             // So the !startsWith check is actually redundant if we trust string ordering properties,
             // but let's keep it implicit or explicit.
             // Actually, "app" < "apple". "apple" starts with "app".
             // Wait, if prefix is "app", and key is "apple". "apple" > "app".
             // If prefix is "apple", and key is "app". "app" < "apple".
             
             // TS: if (data[mid][0] < prefix)
             // If key="app", prefix="apple". "app" < "apple". "app" does not start with "apple". -> left = mid + 1. Correct.
             left = mid + 1;
        } else if (startsWith) {
            firstMatch = mid;
            right = mid - 1;
